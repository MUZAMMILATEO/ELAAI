...
folder_path = "/home/khanm/workfolder/bw/MFA-Net/data/"  # Add the path to your data directory

def load_data(img_height, img_width, images_to_be_loaded, dataset):
    IMAGES_PATH = folder_path + 'imgs/'
    ...
    for n, id_ in tqdm(enumerate(train_ids), total=images_to_be_loaded):
        ...
        # Convert mask to grayscale if it is not already
        if len(mask_.shape) == 3:
            mask_ = Image.fromarray(mask_).convert('L')
